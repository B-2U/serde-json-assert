...
use serde_json_assert::{
    ...
    try_assert_json_matches, CompareMode, Config, FloatCompareMode, NumericMode,
};
...
fn inclusive_match_without_panicking() {
    ...
    assert!(
        try_assert_json_matches(&json!({ "a": 1, "b": 2 }), &json!({ "b": 2}), &config).is_ok()
    );

    assert!(try_assert_json_matches(&json!({ "a": 1, "b": 2 }), &json!("foo"), &config,).is_err());
}

#[test]
fn exact_match_without_panicking() {
    let config = Config::new(CompareMode::Strict).numeric_mode(NumericMode::Strict);
    assert!(try_assert_json_matches(&json!([1, 2, 3]), &json!([1, 2, 3]), &config,).is_ok());

    assert!(try_assert_json_matches(&json!([1, 2, 3]), &json!("foo"), &config).is_err());
}
